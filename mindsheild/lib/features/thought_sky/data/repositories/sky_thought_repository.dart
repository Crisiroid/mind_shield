...
class SkyThoughtRepository extends OfflineFirstRepository<SkyThoughtModel> {
  ...
  @override
  Future<SkyThoughtModel> pushCreate(SkyThoughtModel item) =>
      _remoteDataSource.createSkyThought(thought: item);

  @override
  Future<SkyThoughtModel> pushUpdate(SkyThoughtModel item) => _remoteDataSource
      .updateSkyThought(id: item.id, cloudSwiped: item.cloudSwiped);

  /// Create a new sky thought (offline-first).
  Result<SkyThoughtModel> createSkyThought({required SkyThoughtModel thought}) {
    return writeCreate(
      thought,
  ...
  Result<SkyThoughtModel> updateSkyThought({
    required String id,
