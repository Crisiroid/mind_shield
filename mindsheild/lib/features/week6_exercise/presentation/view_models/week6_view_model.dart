...
class Week6ViewModel extends ChangeNotifier with SubmissionFlow {
  final Week1ExerciseRepository _exerciseRepo;
  final DayProgressRepository _dayProgressRepo;

  Week6ViewModel(this._exerciseRepo, this._dayProgressRepo);
  ...
  // --- Week 6 specific getters ---

  /// Thought observation status from day 37
  Map<String, dynamic>? get thoughtObservation {
    final records = _exerciseResponses
        .where((e) => e.exerciseType == 'guided_thought_observation')
        .toList();
  ...
  /// Optional thought text from day 38
  String? get optionalThoughtText {
    final records = _exerciseResponses
        .where((e) => e.exerciseType == 'thought_labeling')
        .toList();
    if (records.isEmpty) return null;
    try {
      final last = records.last;
      final data = jsonDecode(last.responseData) as Map<String, dynamic>;
      return data['optional_thought_text'] as String?;
    } catch (_) {
      return null;
    }
  }

  /// Emotion registration from day 39
  Map<String, dynamic>? get emotionRegistration {
    final records = _exerciseResponses
        .where((e) => e.exerciseType == 'emotion_identification')
        .toList();
    if (records.isEmpty) return null;
    try {
      final last = records.last;
  ...
  /// Emotion acceptance from day 40
  Map<String, dynamic>? get emotionAcceptance {
    final records = _exerciseResponses
        .where((e) => e.exerciseType == 'emotion_acceptance')
        .toList();
    if (records.isEmpty) return null;
    try {
      final last = records.last;
  ...
  /// Audio exercises count (days 37, 40, 41)
  int get audioExercisesCompleted => _exerciseResponses
      .where(
        (e) =>
            e.exerciseType == 'guided_thought_observation' ||
            e.exerciseType == 'emotion_acceptance' ||
            e.exerciseType == 'pause_and_choose',
      )
      .length;

  /// Registered emotions count
  int get registeredEmotionsCount => _exerciseResponses
      .where((e) => e.exerciseType == 'emotion_identification')
      .length;

  /// Pause exercises count
  int get pauseExercisesCount => _exerciseResponses
      .where((e) => e.exerciseType == 'pause_and_choose')
      .length;
