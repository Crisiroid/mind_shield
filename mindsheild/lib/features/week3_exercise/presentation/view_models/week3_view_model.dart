...
class Week3ViewModel extends ChangeNotifier with SubmissionFlow {
  final Week1ExerciseRepository _exerciseRepo;
  final DayProgressRepository _dayProgressRepo;

  Week3ViewModel(this._exerciseRepo, this._dayProgressRepo);
  ...
  int get thoughtRecordsCount {
    return _exerciseResponses
        .where((e) => e.exerciseType == 'thought_record')
        .length;
  }

  int? get lastBeliefScore {
    final records = _exerciseResponses
        .where(
          (e) =>
              e.exerciseType == 'thought_record' ||
              e.exerciseType == 'thought_belief',
        )
        .toList();
    ...
    try {
      ...
      return (data['belief'] as num?)?.toInt() ??
          (data['belief_score'] as num?)?.toInt();
    } catch (_) {
      return null;
    }
  }

  String? get lastThoughtPattern {
    final records = _exerciseResponses
        .where((e) => e.exerciseType == 'thought_record')
        .toList();
    if (records.isEmpty) return null;
